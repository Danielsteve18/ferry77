...
const NewRequest: React.FC<NewRequestProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    if (tourParam === 'quick-request' && continueParam === 'true') {
      // Esperar a que el componente esté completamente renderizado
      const timeoutId = setTimeout(() => {
        console.log('🎯 Iniciando tour automáticamente en NewRequest...');
        startNewRequestPageTour();
        
        // Limpiar los parámetros de la URL para evitar que se ejecute múltiples veces
        const newUrl = window.location.pathname;
        window.history.replaceState({}, document.title, newUrl);
      }, 1000); // Dar tiempo para que los elementos se rendericen
      
      return () => clearTimeout(timeoutId);
    }
  }, [location.search, startNewRequestPageTour]);
    ...
    try {
      ...
      const selectedCompanies = bestQuotes
        ...
        .map(quote => {
          ...
          console.log(`[NewRequest] Verificando nombre de empresa para ${quote.name}: ${(company as any).companyName}`);
          if (!(company as any).companyName) {
      ...
      const selectedCompanyIds = selectedCompanies.map((company: any) => company.companyId || company.id);
      
      // Verificar la integridad de los datos antes de guardar
      const validatedSelectedCompanies = selectedCompanies.map((company: any) => {
        ...
        if (result.success && result.solicitud) {
          ...
          for (const company of validatedSelectedCompanies) {
            try {
              ...
              logger.debug('NewRequest', `Notificación enviada a empresa ${(company as any).id}`);
            } catch (error) {
              console.error(`[NewRequest] Error al enviar notificación a empresa ${(company as any).id}:`, error);
            }
          }
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-2xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <Link 
              ...
              className="flex items-center space-x-2 px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-300 rounded-lg transition-colors text-sm font-medium"
            >
              <ArrowLeft size={16} />
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Información General */}
          <div className="bg-white rounded-xl shadow-sm p-6" data-tour="request-form">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6" data-tour="upload-files">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6" data-tour="text-description">
          ...
          {/* Submit */}
          <div className="flex space-x-3" data-tour="preview-request">
            <Button 
              type="submit" 
